...
void Keyboard::releaseALL(){
    for (std::map<char,int>::iterator mapIt = keys.begin(); mapIt != keys.end(); mapIt++)
    {
        ip.ki.wVk = mapIt->second;
        ip.ki.dwFlags = KEYEVENTF_KEYUP;
        SendInput(1, &ip, sizeof(INPUT));
    }
}
